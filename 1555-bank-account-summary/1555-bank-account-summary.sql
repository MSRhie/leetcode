...
SELECT
    ...
    CASE WHEN IFNULL(SUM(CASE WHEN A.user_id = B.paid_by THEN -amount ELSE amount END ),0) + A.credit > 0 THEN 'No' ELSE 'Yes' END AS credit_limit_breached
FROM Users AS A
LEFT JOIN Transactions AS B
ON A.user_id = B.paid_by OR A.user_id = B.paid_to # user_id가 어떠한 행위를 하거나 당했을 때 (user_id기준)
GROUP BY user_id # SUM() 하기 위해서